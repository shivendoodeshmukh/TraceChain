...
installChaincodes() {
            if [ -n "$(ls "$CHAINCODES_BASE_DIR/./chaincodes/rawMaterialTokenContract")" ]; then
                              local version="0.0.1"
            printHeadline "Packaging chaincode 'rawMaterialTokenContract'" "U1F60E"
chaincodeBuild   "rawMaterialTokenContract"   "golang"   "$CHAINCODES_BASE_DIR/./chaincodes/rawMaterialTokenContract"   "16"
...
chaincodeCommit   "cli.supplier.example.com"   "peer0.supplier.example.com:7041"   "supply"   "rawMaterialTokenContract"   "$version"   "orderer0.group1.orderer.example.com:7030"   ""   "false"   "crypto-orderer/tlsca.orderer.example.com-cert.pem"   "peer0.supplier.example.com:7041,peer0.manufacturer.example.com:7061"   "crypto-peer/peer0.supplier.example.com/tls/ca.crt,crypto-peer/peer0.manufacturer.example.com/tls/ca.crt"   ""
        ...
        echo "Looked in dir: '$CHAINCODES_BASE_DIR/./chaincodes/rawMaterialTokenContract'"
      fi
...
runDevModeChaincode() {
  ...
  chaincodeApprove     "cli.supplier.example.com"     "peer0.supplier.example.com:7041"     "supply"     "rawMaterialTokenContract"     "0.0.1"     "orderer0.group1.orderer.example.com:7030"     ""     "false"     ""     ""
  printHeadline "Approving 'rawMaterialTokenContract' for Manufacturer (dev mode)" "U1F60E"
  chaincodeApprove     "cli.manufacturer.example.com"     "peer0.manufacturer.example.com:7061"     "supply"     "rawMaterialTokenContract"     "0.0.1"     "orderer0.group1.orderer.example.com:7030"     ""     "false"     ""     ""
printItalics "Committing chaincode 'rawMaterialTokenContract' on channel 'supply' as 'Supplier' (dev mode)" "U1F618"
chaincodeCommit   "cli.supplier.example.com"   "peer0.supplier.example.com:7041"   "supply"   "rawMaterialTokenContract"   "0.0.1"   "orderer0.group1.orderer.example.com:7030"   ""   "false"   ""   "peer0.supplier.example.com:7041,peer0.manufacturer.example.com:7061"   ""   ""
...
networkDown() {
  ...
  printf "Removing chaincode containers & images... \U1F5D1 \n"
                            for container in $(docker ps -a | grep "dev-peer0.supplier.example.com-rawMaterialTokenContract" | awk '{print $1}'); do echo "Removing container $container..."; docker rm -f "$container" || echo "docker rm of $container failed. Check if all fabric dockers properly was deleted"; done
        for image in $(docker images "dev-peer0.supplier.example.com-rawMaterialTokenContract*" -q); do echo "Removing image $image..."; docker rmi "$image" || echo "docker rmi of $image failed. Check if all fabric dockers properly was deleted"; done
                      for container in $(docker ps -a | grep "dev-peer1.supplier.example.com-rawMaterialTokenContract" | awk '{print $1}'); do echo "Removing container $container..."; docker rm -f "$container" || echo "docker rm of $container failed. Check if all fabric dockers properly was deleted"; done
        for image in $(docker images "dev-peer1.supplier.example.com-rawMaterialTokenContract*" -q); do echo "Removing image $image..."; docker rmi "$image" || echo "docker rmi of $image failed. Check if all fabric dockers properly was deleted"; done
                                for container in $(docker ps -a | grep "dev-peer0.manufacturer.example.com-rawMaterialTokenContract" | awk '{print $1}'); do echo "Removing container $container..."; docker rm -f "$container" || echo "docker rm of $container failed. Check if all fabric dockers properly was deleted"; done
        for image in $(docker images "dev-peer0.manufacturer.example.com-rawMaterialTokenContract*" -q); do echo "Removing image $image..."; docker rmi "$image" || echo "docker rmi of $image failed. Check if all fabric dockers properly was deleted"; done
